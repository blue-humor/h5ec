...
const Index: React.FC<IndexProps> = (props) => {
  ...
  return (
    <>
        ...
        >
          {
            images?.map((item, index) => {
              return <Swiper.Item key={index}>
                <Image lazyload src={item} key={index} onClick={() =>
                  ImagePreview.open({
                    images,
                  })
                } />
        ...
        <Card >
          <Card.Body>
            <Flex justify="between" align="center">
              <Flex.Item>
                <Typography.Text type="danger"><span className={styles.card_currentPrice}>¥{299}</span>起</Typography.Text>{' '}
                <Typography.Text delete className={styles.card_originalPrice}>¥{499}</Typography.Text>
              </Flex.Item>
              <Flex.Item>
          ...
          <Card.Header>
            <Typography.Title level={6} ellipsis={{
              rows: 2,
              expandText: '更多',
            }}>React Vant 是一套轻量、可靠的移动端 React 组件库，提供了丰富的基础组件和业务组件动应用，使用过程中发现任何问题都可以提 Issue 给我们，当然，我们也非常欢迎你给我们发 PR。</Typography.Title>
          </Card.Header>
        </Card>
      ...
      <GoodsSku showShoppin={showShopping} handleShowShopping={handleShowShopping} showButtonType={showButtonType} />
